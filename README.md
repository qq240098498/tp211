...
- **偏差分析**：按指令与时段对比要求值与实际值，给出绝对偏差、相对偏差与偏差水量，按放多了、放少了、时段错开了分类，并按类合计条数与偏差。
...
7. **偏差分析**：要求值 = 指令的目标下泄流量；实际值 = 指令时段（**含首尾两天**）内出库流量记录的算术均值。绝对偏差 = 实际值 − 要求值；相对偏差 = 绝对偏差 ÷ 要求值 × 100%。相对偏差的绝对值不超过 `deviationTolerancePct`（默认 5%）判为基本符合，超出按方向判为放多了或放少了。时段内有出库记录的天数不足一半、且时段前后 `deviationShiftDays`（默认 7）天内有出库记录的，判为时段错开了；前后也没有记录的判为无数据。偏差水量 = 绝对偏差 × 时段天数 × 86400 ÷ 10000（万 m³）。分类卡片上的合计偏差与合计偏差水量按分类求和（水库与状态筛选参与求和，分类筛选只影响明细行）。
...
| GET | /api/deviation | 指令偏差分析（支持 reservoirId、status、category），含逐条偏差与分类合计 |
