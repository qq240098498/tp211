...
- **概览**：水库数、今日各库水位与限水位、超限记录数、指令按状态、放多/放少指令数、预警等级。
- **水库**：水库台账（水位口径、曲线点数、库容对不上时的提示）、水位-库容曲线的维护与查询。
- **水位与流量**：水位记录、入库流量、出库流量的登记与查询。
- **调度指令**：指令的下达、修改、复制、撤销、删除与附件的登记；列表附实际均值、绝对/相对偏差与偏差类别。
- **偏差分析**：按指令与执行时段对齐出库记录，给每条指令算要求值、实际均值、绝对偏差、相对偏差、偏差水量，按方向与时段覆盖分类（放多了、放少了、时段错开了、基本一致、时段未结束、暂无实测、已撤销），页面按类别给出条数与合计偏差。
...
7. **下泄偏差分析**：按指令执行时段（`windowStart` 至 `windowEnd`，含两端）逐日对齐出库流量，同一天多条先取日均值，实际均值取有实测各天日均值的平均。绝对偏差 = 实际均值 − 要求值（正为放多、负为放少）；相对偏差 = 绝对偏差 ÷ 要求值；偏差水量（万m³）= 绝对偏差 × 有实测天数 × 86400 ÷ 10000（缺测日不外推）。分类顺序：已撤销不考核 → 时段末日晚于今天为「时段未结束」→ 时段内无实测为「暂无实测」→ 实测天数不足时段一半为「时段错开了」→ 绝对偏差超过 `flowDeviationTolerance`（默认 5 m³/s）或相对偏差超过 `flowDeviationPercent`（默认 10%）按方向分「放多了 / 放少了」→ 其余为「基本一致」。
...
| GET | /api/health | 健康检查 |
| GET | /api/summary | 概览 |
| GET / PATCH | /api/settings | 全局设置（汛期起止、损失、容差、流量门槛、下泄偏差容差等） |
...
| GET | /api/deviations?reservoirId=&category= | 下泄偏差分析（逐条要求值/实际值/绝对偏差/相对偏差/偏差水量、类别、分类合计） |
